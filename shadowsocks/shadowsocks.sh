...
#   Intro:  http://clangcn.com/blog/
#===============================================================================================

clear
function clangcn.com(){
echo ""
echo "#############################################################"
echo "# One click Install Shadowsocks(Python)"
echo "# Intro: http://clangcn.com/blog/"
echo "#"
...
function pre_install(){
    ...
    echo "Please input port for shadowsocks:"
    read -p "(Default port: 8388):" shadowsocksport
    if [ "$shadowsocksport" = "" ]; then
        shadowsocksport="8388"
    fi
    echo -e "Your Server Port: \033[32m \033[01m${shadowsocksport}\033[0m"
    echo "####################################"
    #Set shadowsocks config password
    echo "Please input password for shadowsocks:"
    read -p "(Default password: ilovechina):" shadowsockspwd
    ...
    if [ "$OS" == 'CentOS' ]; then
        yum install -y wget nano unzip openssl-devel gcc swig python python-devel python-setuptools autoconf libtool libevent
        yum install -y automake make curl curl-devel zlib-devel perl perl-devel cpio expat-devel gettext-devel
    else
        apt-get -y update
        apt-get -y install python python-dev python-pip curl wget nano unzip gcc swig automake make perl cpio
    fi
    # Get IP address
...
function install_ss(){
    ...
    if [ -f /usr/bin/pip ]; then
        ...
        if [ -f /usr/bin/ssserver ] || [ -f /usr/local/bin/ssserver ]; then
            ...
            echo ""
            echo "Shadowsocks install failed! Please visit http://clangcn.com/blog/ and contact."
            exit 1
        fi
        clear
        clangcn.com
        ...
        echo ""
        echo "Welcome to visit:http://clangcn.com/blog/"
        echo "Enjoy it!"
        echo ""
        exit 0
    else
        echo ""
        echo "pip install failed! Please visit http://clangcn.com/blog/ and contact."
        exit 1
    fi
}
...
case "$action" in
install)
    install_shadowsocks 2>&1 | tee $HOME/install_shadowsocks.log
    ;;
uninstall)
    uninstall_shadowsocks 2>&1 | tee $HOME/uninstall_shadowsocks.log
    ;;
*)
    clangcn.com
